"""
Sync two folders from systray V0.38 April 2020
...
def run_at_boot(stray):
    ...
    # If short-cut found in startup folder, delete it and return.
    if os.path.exists('C:/Users/All Users/Microsoft/Windows/Start Menu/Programs/Startup/sync-it-v038 - Shortcut.lnk'):
        os.remove('C:/Users/All Users/Microsoft/Windows/Start Menu/Programs/Startup/sync-it-v038 - Shortcut.lnk')
        messagebox.showinfo('Sync It',
                            '\n\nSync It V0.38 short cut now removed from startup.\n\n'
                            'To reverse this action call this option again.')
        return

    # Check for exe in cwd, if not found return.
    if not os.path.exists('sync-it-v038.exe'):
        messagebox.showerror('ERROR',
                             'sync-it-v038.exe absent from current path.\n\n'
                             'Cannot perform action.')
        return

    # Check for short-cut in cwd, if not found return.
    if not os.path.exists('sync-it-v038 - Shortcut.lnk'):
        messagebox.showerror('ERROR',
                             '"sync-it-v038 - Shortcut.lnk"\n'
                             'not found in current dir.\n\n'
                             'Cannot perform action.')
        return

    # Copy short-cut to startup folder.
    if not os.path.exists('C:/Users/All Users/Microsoft/Windows/Start Menu/Programs/Startup/sync-it-v038 - Shortcut.lnk'):
        copy(getcwd()+'/sync-it-v038 - Shortcut.lnk', 'C:/Users/All Users/Microsoft/Windows/Start Menu/Programs/Startup')
        messagebox.showinfo('Sync It',
                            '\n\nSync It V0.38 short-cut copied to startup folder.\n\n'
...
def msg_box(stray):
    ...
    Glo.custom_mbox.title('Sync It V0.38. ')
